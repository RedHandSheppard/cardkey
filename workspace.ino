...
const String CANCEL_SIGNAL = "!!CANCEL_PROMPT!!";
...
void drawColorMenu() {
    ...
    for (int i = 0; i < MAX_VISIBLE_BUTTONS; i++) {
        ...
        if (itemIndex == 1) {
            uint8_t r8, g8, b8;
            RGB565toRGB888(MENU_HIGHLIGHT_COLOR, r8, g8, b8);
            char hex_code[7];
            sprintf(hex_code, "%02X%02X%02X", r8, g8, b8);
            display_label = display_label + " #" + String(hex_code);
        } else if (!is_action) {
...
bool handleMenuAction(State state, int index) {
    if (state == STATE_TOP_MENU) {
        ...
        if (index == 0) {
            ...
            if (hexInput == CANCEL_SIGNAL) {
                if (currentState != STATE_KEYBOARD) { drawColorMenu(); resetMenuTimer(); }
                return true;
            }
        ...
        if (index == 0) {
            ...
            if (label == CANCEL_SIGNAL) {
                if (currentState != STATE_KEYBOARD) { drawPassMenu(); resetMenuTimer(); }
                return true;
            }
        ...
        if (index == 0) {
            ...
            if (label == CANCEL_SIGNAL) {
                if (currentState != STATE_KEYBOARD) { drawNoteMenu(); resetMenuTimer(); }
                return true;
            }
...
void runPrompt() {
    ...
    if (input.length() > 0 && input != CANCEL_SIGNAL) {
